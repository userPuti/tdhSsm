...
$(function(){
	...
	mygrid.setHeader('序号,案号,案件名称,承办部门,承办人,标记,操作');
	mygrid.setInitWidths('100,200,*,200,200,100,250');
    mygrid.setColumnMinWidth(200, 2);
	mygrid.setColTypes('ro,ro,ro,ro,ro,img,btns');
	mygrid.setColAlign('center,center,left,center,center,center,left');
...
function eXcell_btns(cell){
    ...
    this.setValue = function(val){
		...
		if(moreAry.length){
			...
			if(!popObj.length) $('body').append('<div data-dropdom="more" class="droppop" style="width:150px"><ul class="droppop_list"></ul></div>');
		}
		html += '</div>';
        this.setCValue(html);
    }
