...
function dhtmlXGridObject(a) {
	...
	this.init = function(e) {
		...
		if(this._hstyles) {
			...
			_isIE && _isIE < 8 && document.documentMode < 8 && document.body.style.msTouchAction == this.undefined ? b.style.position = "absolute" : b.style.height = "auto";
			...
			if(!h.firstChild) {
				...
				h = h.firstChild;
				_isIE && _isIE < 8 && document.documentMode < 8 && document.body.style.msTouchAction == this.undefined ? h.style.position = "absolute" : h.style.height = "auto";
...
dhtmlXGridObject.prototype = {
	//分页
	setPageCurrent: function(a){
		if(!a) return false;
		if(isNaN(a)) return false;
		this.setPageCurrent = parseInt(a);
	},
	pagingLayout: function(str){
		if(!str) return false;
		this.pagingLayout = str.split(",");
	},
	pagingLimits: function(str){
		if(!str) return false;
		this.pagingLimits = str.split(",");
	},
	enablePagingon: function(a,b,c){
		if(a){
			...
			this.pagingLayout = "function" != typeof this.pagingLayout ? this.pagingLayout : ['prev', 'page', 'next', 'count'];
			this.pagingLimits = "function" != typeof this.pagingLimits ? this.pagingLimits : (this.pageLimit + "").split();
			var limitFlag = 0;
			for(z in this.pagingLimits){
	    		if(this.pagingLimits[z] == this.pageLimit){
	    			limitFlag += 1;
	    		}
	    	}
			if(limitFlag < 1){
				this.pagingLimits.unshift(this.pageLimit + "");
			}
			for(k in this.pagingLayout){
				switch(this.pagingLayout[k]) {
				    case "count":
				    	this.pageToggleObj = document.createElement("span");
				    	this.pageToggleObj.className = "page_toggle";
				        this.pageToggleObj.innerHTML = "共"+this.pageTotal+"条";
				        this.pageBox.appendChild(this.pageToggleObj);
				        break;
				    case "first":
				    	this.pageFirstObj = document.createElement("a");
				    	this.pageFirstObj.className = "page_first page_first_dis";
				    	this.pageBox.appendChild(this.pageFirstObj);
				        break;
				    case "last":
				    	this.pageLastObj = document.createElement("a");
				    	this.pageLastObj.className = "page_last";
				    	this.pageBox.appendChild(this.pageLastObj);
				        break;
				    case "prev":
				    	this.pagePrevObj = document.createElement("a");
				    	this.pagePrevObj.className = "page_prev page_prev_dis";
				    	this.pageBox.appendChild(this.pagePrevObj);
				        break;
				    case "next":
				    	this.pageNextObj = document.createElement("a");
				    	this.pageNextObj.className = "page_next";
				    	this.pageBox.appendChild(this.pageNextObj);
				        break;
				    case "limit":
				    	this.pageLimitObj = document.createElement("span");
				    	this.pageLimitObj.className = "page_limit";
				    	var optionsHtml = "<option value=''></option>";
				    	//for(g in this.pagingLimits){ 
				    	//for..in循环会把某个类型的原型(prototype)中方法与属性给遍历出来
				    	for(var g = 0;g < this.pagingLimits.length; g++){
				    		optionsHtml += "<option value='" + this.pagingLimits[g] + "'>" + this.pagingLimits[g] + "条/页</option>";
				    	}
				    	this.pageLimitObj.innerHTML = "<select class='inputSel' style='font-size:12px;'>"+ optionsHtml +"</select>";
				    	this.pageBox.appendChild(this.pageLimitObj);
				        break;
				    case "refresh":
				    	this.pageRefreshObj = document.createElement("a");
				    	this.pageRefreshObj.className = "page_refresh";
				    	this.pageBox.appendChild(this.pageRefreshObj);
				        break;
				    case "skip":
				    	this.pageSkipObj = document.createElement("span");
				    	this.pageSkipObj.className = "page_skip";
				    	this.pageSkipObj.innerHTML += "<span class='page_span'>跳转至</span>"+
									"<input class='page_to' type='text' onkeyup=\"value=value.replace(/^(0+)|[^\\d]+/g,'')\" />"+
									"<span class='page_span'>页</span>"+
									"<a class='page_btn'>确定</a>";
				    	this.pageBox.appendChild(this.pageSkipObj);
				        
				        break;
				    case "page":
				    	this.pagePageObj = document.createElement("span");
				    	this.pagePageObj.className = "page_page";
				        this.pagePageObj.innerHTML = "第"+this.pageCurrent+"页，共"+this.pageCount+"页";
				        this.pageBox.appendChild(this.pagePageObj);
				}
			}					
			...
			if(this.pageLimitObj){
				selectInit(jQuery(this.pageLimitObj).find('select'));
				setSelVal(jQuery(this.pageLimitObj).find('select'), this.pageLimit);
			}
			this.doPageEvent();
		}
	},
	...
	doChangePage:function(){
		...
		if(this.pageTotal > 0 && (this.pageCurrent > this.pageCount || this.pageCurrent < 1)){
			this.pageCurrent = 1;
			this.loadPage();
			return false;
		}
		this.pagePageObj.innerHTML = "第"+this.pageCurrent+"页，共"+this.pageCount+"页";
		this.pageToggleObj.innerHTML = "共"+this.pageTotal+"条";
		if(typeof this.pageNextObj == "object"){
			this.pageNextObj.className = this.pageNextObj.className.replace("page_next_dis", "");
		}
		if(typeof this.pageLastObj == "object"){
			this.pageLastObj.className = this.pageLastObj.className.replace("page_last_dis", "");
		}
		if(typeof this.pageFirstObj == "object"){
			this.pageFirstObj.className = this.pageFirstObj.className.replace("page_first_dis", "");
		}
		if(typeof this.pagePrevObj == "object"){
			this.pagePrevObj.className = this.pagePrevObj.className.replace("page_prev_dis", "");
		}
		if(this.pageCurrent == 1){
			if(typeof this.pageFirstObj == "object"){
				this.pageFirstObj.className += " page_first_dis";
			}
			if(typeof this.pagePrevObj == "object"){
				this.pagePrevObj.className += " page_prev_dis";
			}
		}
		if(this.pageCurrent == this.pageCount){
			if(typeof this.pageNextObj == "object"){
				this.pageNextObj.className += " page_next_dis";
			}
			if(typeof this.pageLastObj == "object"){
				this.pageLastObj.className += " page_last_dis";
			}
		}
	...
	doPageEvent:function(){
		...
		that.pageBox.onclick=function(a){
			...
			if(d.className.indexOf("page_prev") > -1){
				if(d.className.indexOf("page_prev_dis") > -1) return false;
				that.pageCurrent--;
				if(that.pageCurrent < 1){
					that.pageCurrent = 1;
					return false;
				}
			}else if(d.className.indexOf("page_next") > -1){
				if(d.className.indexOf("page_next_dis") > -1) return false;
				that.pageCurrent++;
				if(that.pageCurrent > that.pageCount){
					that.pageCurrent = that.pageCount;
					return false;
				}
			}else if(d.className.indexOf("page_first") > -1){
				if(that.pageCurrent == 1) return false;
				that.pageCurrent = 1;
			}else if(d.className.indexOf("page_last") > -1){
				if(that.pageCurrent == that.pageCount) return false;
				that.pageCurrent = that.pageCount;
			}else if(d.className.indexOf("page_btn") > -1){
				var pageValueObj = that.pageSkipObj.getElementsByTagName('input')[0],
					pageToVal = pageValueObj.value;
				if(!!pageToVal){
					if(isNaN(pageToVal)){
			...
			}else if(d.className.indexOf("page_refresh") > -1){
				
			}else if(d.className.indexOf("page_toggle") > -1){
				that.pageCurrent = 1;
				that.pageLimit = that.pageTotal;
				if(that.pageLimitObj){
					setSelVal($(that.pageLimitObj.getElementsByTagName('select')[0]),'');
				}
			}else{
				return false;
			}
		...
		if(that.pageLimitObj){
			that.pageLimitObj.getElementsByTagName('select')[0].onchange = function(){
				if(this.value == ''){
					that.pageLimit = that.pageTotal;
				}else{
	      			that.pageLimit = this.value;
				}
				that.pageCurrent = 1;
				that.loadPage();
			}
	...
	_createHRow: function(a, b) {
		if(!b) {
			...
			_isIE && _isIE < 8 && document.documentMode < 8 ? e.style.position = "absolute" : e.style.height = "auto"
		}
		var f = a[1],
	...
	loadXML: function(a, b) {
		if(this.pageFlag){
			this.pageUrl = a;
			this.pageCallback = b;
			this.pageCurrent = "function" != typeof this.setPageCurrent ? this.setPageCurrent : 1;
			this.loadPage(true);
		}else{
			this.load(a, b, "xml");
		}
